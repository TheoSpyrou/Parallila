...
int main(int argc , char** argv){
	...
	do {
		...
		cudaMeanshift1<<<numBlocks, threadsPerBlock>>>(dYv, dXv, dDenominator, dNumerator, dimensions);
...
__global__ void cudaMeanshift1(float* y, float* x, float* denom, float* numer, dim3 dim){
	...
	unsigned short colZ = blockDim.x * blockIdx.x + threadIdx.x;
	unsigned short rowZ = blockDim.y * blockIdx.y + threadIdx.y;
	unsigned short rowX = colZ * dim.z;
	unsigned short rowY = rowZ * dim.z;
	...
	//and data set size
	if (colZ < dim.x && rowZ < dim.y){
		//Calculate difYiXj
		for (short i = 0; i < dim.z; i++)
			difYiXj += powf(y[rowY + i] - x[rowX + i], 2.0f);
		...
		if (sqrtf(difYiXj) <= sigma * sigma){
			...
			atomicAdd(denom + rowZ, gauss);
			for (short i = 0; i < dim.z; i++)
				atomicAdd(numer + rowZ * dim.z + i, gauss * x[rowX + i]);
		}
	}
}
